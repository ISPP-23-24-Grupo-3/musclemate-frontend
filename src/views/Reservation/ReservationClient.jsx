...
import { getFromApi, postToApi, deleteFromApi } from "../../utils/functions/api";
import {
    Button,
    Heading,
    Select,
...
export default function ReservationClient() {
    ...
    useEffect(() => {
        const fetchReservationClientEvent = async () => {
            try {
                const response = await getFromApi(`reservations/byClientEvent/${eventId}/`);
                if (response.ok) {
                    const data = await response.json();
    ...
    const handleDelete = async () => {
        try {
            const response = await deleteFromApi(`reservations/delete/${reservation.id}/`);
            if (response.ok) {
    ...
    return (
        <div className="max-w-xl mx-auto">
            ...
            <div>
                ...
                {!reservation && eventDetails.isClickable && (eventDetails.capacity > eventDetails.attendees) && (
                    <FormContainer>
                        <Heading size="5" className="text-radixgreen !mb-3 text-center">
